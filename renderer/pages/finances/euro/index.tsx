...
const EuroPage = () => {
  const url = useAppSelector((state) => state.resourceUrls.transactionsEUR.fetchedUrl);
  ...
  const renderPage = () => {
    ...
    if (data) {
      ...
      return (
        <>
          <FiatTable transactions={data.fiat_transactions} resource="transactionsEUR" />
          <Pagination resource="transactionsEUR" results={data.results} />
        </>
      );
    }
  };
    ...
    <>
      ...
      <>
        <PageHeader
          ...
          CTAText="Ajouter"
          onCTAClick={() => {
            dispatch(addModal({ name: "transactionsEUR", title: "Ajouter un virement euro" }));
          }}
        >
          <div style={{ display: "flex", gap: "2rem" }}>
            <FiatFilter resource="transactionsEUR" />
