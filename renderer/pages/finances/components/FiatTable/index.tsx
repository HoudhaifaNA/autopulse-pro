...
const FiatTable = ({ transactions, resource }: TransactionsTableProps) => {
  ...
  const pageTransctionsIds = transactions.map((transaction) => transaction.id);
    ...
    return transactions.map((transaction, ind) => {
      ...
      return (
        <T.TableRow key={id} onClick={(event) => event.ctrlKey && checkId(id)}>
          ...
          <T.TableCell>{formattedTransctionAmount}</T.TableCell>
    ...
    <T.TableWrapper>
      <T.Table>
        <T.TableHead>
          <T.TableRow>
            <T.TableHeaderCell>
              <Checkbox isChecked={isAllTransactionsOnPageSelected && transactions.length > 0} check={checkAllOnPage} />
            </T.TableHeaderCell>
