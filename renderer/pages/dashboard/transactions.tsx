...
import { GetTransactionsStatsResponse } from "types";
import { useAppSelector } from "store";

const TransactionsStats = () => {
  const { fetchedUrl } = useAppSelector((state) => state.resourceUrls.transactionsStats);

  const { data, isLoading, error } = useSWR<GetTransactionsStatsResponse>(fetchedUrl, fetcher);
  ...
  const renderPage = () => {
    ...
    if (data) {
      const { transactions_total_amount } = data;

      const renderTransactionsTypes = () => {
        return transactions_total_amount.map((transaction) => {
