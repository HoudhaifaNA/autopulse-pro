...
import { GetAllLicencesResponse } from "types";
import LicencesFilter from "pages/licences/components/LicencesFilter";
import { addModal } from "store/reducers/modals";

const LicencesPage = () => {
  const url = useAppSelector((state) => state.resourceUrls.licences.fetchedUrl);
  const selectedIds = useAppSelector((state) => state.selectedItems.selectedIds);
  const { data, isLoading, error } = useSWR<GetAllLicencesResponse>(url, fetcher);
    ...
    if (data) {
      ...
      return (
        <>
          ...
          <Pagination resource="licences" results={data.results} />
        </>
      );
    }
  };
    ...
    <>
      ...
      <>
        <PageHeader
          ...
          CTAText="Ajouter"
          onCTAClick={() => {
            dispatch(addModal({ name: "licences", title: "Ajouter une licence" }));
          }}
        >
          <div style={{ display: "flex", gap: "2rem" }}>
            <LicencesFilter />
            <Body1>We found {data?.results} licences</Body1>
          </div>
        </PageHeader>
